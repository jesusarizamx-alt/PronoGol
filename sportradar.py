...
class SportradarScraper:
    def __init__(self, api_key=None, db=None):
        ...
        # 3. Base de datos — busca cualquier key cuyo nombre contenga "sportradar"
        self.api_key = api_key or os.environ.get('SPORTRADAR_API_KEY') or self._find_key_in_db()

    def _find_key_in_db(self):
        """Busca en la DB cualquier key que tenga 'sportradar' en el nombre (sin importar mayúsculas)."""
        if not self.db:
            return None
        try:
            ...
            for name, value in all_keys.items():
                if 'sportradar' in name.lower() and value:
                    print(f"[Sportradar] ✅ API key encontrada en DB con nombre: '{name}'")
                    return value
        except Exception:
            pass
        return None
    ...
    def get_live_matches(self):
        ...
        Usa schedule.json (compatible con trial) — results.json no existe en trial.
        """
        data = self._get("schedules/live/schedule.json")
        if not data:
            return []
        results = []
        # schedule.json devuelve sport_events directo (no anidados en results)
        for sched in data.get('sport_events', []):
            # Puede ser dict plano o anidado en sport_event
